...
```
##### 1.2.1.2 Підключаємо телеграм (крок можна пропустити, якщо telegram-ingestion-service читає з mock-джерела). Запускаємо команду одноразово, щоб підключити сервіс до телеграму. Після вводу номеру телефону і одноразового коду - зупиняємо виконання.
```bash
